...
### On this page:
  - [CAN bus not setup properly](#can-bus-not-setup-properly)
...
Try: 
 - unplugging and replugging can adapter.  
 - restarting services one by one (restarting them all together could throw errors). 

Most USB-to-CAN adapters have lights that indicate when CAN messages are being transmitted. If you see them blinking rapidly, it's likely your CAN connection works!
...
if you don't see 706 or 707, your setup scripts are not launching the dbus clients correctly. We haven't encountered this issue, but check if your services are running: 
```bash
...
```
They're always there even when it works. Look for things like SDO timouts after 5 retry's, etc. 



## Motors are assigned to the same CAN NodeID

This is an easy fix. But first, to confirm your nodes are assigned to the same ID's check:
```bash
candump can0 | grep 7 
```
if you only see one of 704 or 705 and don't see the other, this means likely your motors share the same NodeID. 
...
Re-run the script for the wheel that appears entirely green (OK). Watch the flashing lights on the wheels very closely and press "m", then "o", and then "a". If you see both drivers lights stabilize, this means your commands reached both wheels and enabled NMT and PDS and your motors definetley have the same NodeID.  

<br>

To fix this, we will unplug the CANopen wire of the correct wheel from the SWD Safety Hub. To figure out which wire goes to which motor, look at page 15 of this [manual](https://www.ez-wheel.com/storage/upload/pdf/swd-starter-kit-v5-04042024-instruction-manual-en.pdf).  
...
Make sure you followed Step 5 in Installation process. It's never to late to configure the scripts correctly. If doing that and restarting the computer doesn't work, try commenting out these lines in the commissioning scripts for both wheels:
